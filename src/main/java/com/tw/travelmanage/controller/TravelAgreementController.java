...
public class TravelAgreementController {
    ...
    @ApiOperation(value = "固定费用支付接口", notes = "通过固定费用缴费单ID进行支付", httpMethod = "POST")
    @PostMapping("/fixedfee/{fid}")
    ...
    @ApiOperation(value = "固定费用发票接口", notes = "根据缴费单ID以及发票信息申请发票", httpMethod = "POST")
    @PostMapping("/fixed_invoice/{fid}")
